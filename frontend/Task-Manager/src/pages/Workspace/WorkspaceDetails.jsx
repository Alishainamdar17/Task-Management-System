...
export default function WorkspaceDetails() {
  ...
  // workspace id can come as :workspaceId OR :id depending on route
  const workspaceId =
    params.workspaceId || params.id || params?.workspace || null;
  ...
  const fetchWorkspace = async () => {
    ...
    try {
      const path =
        buildPath(API_PATHS.WORKSPACES?.GET_BY_ID, workspaceId, ":id") ||
      ...
      console.error(
        "[WorkspaceDetails] fetchWorkspace error:",
  ...
  const fetchProjects = async () => {
    ...
    // Primary: workspace-based endpoint
    let path =
      buildPath(
        API_PATHS.PROJECTS?.GET_BY_WORKSPACE,
        workspaceId,
        ":workspaceId"
    ...
    try {
      ...
      try {
        ...
        // Fallback: generic /api/projects?workspace=...
      ...
      console.error(
        "[WorkspaceDetails] fetchProjects error:",
        err?.response?.data || err.message
      );
      setProjects([]);
    }
          ...
          members.slice(0, 5).map((m, i) => {
            ...
            return (
              <div
                key={i}
                title={name}
                className="w-7 h-7 rounded-full bg-sky-600 text-white flex items-center justify-center text-xs font-medium"
              >
                {initial}
              </div>
            );
  ...
  // For sidebar active highlight: use correct path depending on role
  const activeMenuKey =
  ...
  return (
    <DashboardLayout activeMenu={activeMenuKey}>
      <div className="p-6">
        {/* Create Project Modal (Admin + User both can use it) */}
        <CreateProjectModal
          open={createOpen}
        ...
        {loading ? (
          ...
          <>
            {/* HEADER: Workspace Info + New Project Button */}
            <div className="flex items-start justify-between mb-6">
              <div className="flex items-start gap-4">
                <div
                  className="w-12 h-12 rounded-lg flex items-center justify-center text-white font-bold text-lg"
                  style={{
                    backgroundColor: workspace.color || "#F97316",
                  }}
                >
                  {workspace.name
                    ? String(workspace.name).charAt(0).toUpperCase()
                    : "W"}
                </div>
                <div>
              ...
              {/* ⭐ New Project button visible for BOTH admin + user */}
              <div className="flex items-center gap-3">
                <button
                  onClick={() => {
                    // TODO: implement invite flow later (optional)
                  }}
              ...
              {projects.length === 0 ? (
                <div className="rounded-xl border border-gray-100 bg-white p-10 flex flex-col items-center justify-center text-center text-gray-500">
                  ...
                  <p className="text-sm mb-4">
                    Get started by creating your first project in this
                    workspace
                  </p>
                  <button
                    onClick={openCreate}
